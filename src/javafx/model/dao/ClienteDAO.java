   ...
   public boolean remover(Cliente cliente) {
    ...
    try {
        ...
        if (resultado.next() && resultado.getInt(1) > 0) {
            ...
            // Se o cliente tiver registros relacionados, exclui os registros antes de excluir o cliente
            String sql3 = "DELETE FROM agendamentos WHERE cdCliente=?";
            PreparedStatement stmtsql3 = connection.prepareStatement(sql3);
            stmtsql3.setInt(1, cliente.getCdCliente());
            stmtsql3.executeUpdate();
        }
    ...
    public Cliente buscarPorVeiculo(Veiculo veiculo) {
    String sql = "SELECT * FROM clientes WHERE cdCliente IN (SELECT cdCliente FROM veiculos WHERE cdVeiculo = ?)";
    ...
    try {
        ...
        if (resultado.next()) {
            ...
            cliente.setCdCliente(resultado.getInt("cdCliente"));
            cliente.setNome(resultado.getString("nome"));
            // Preencha os demais atributos do cliente, se necessário
        }
    } catch (SQLException ex) {
        Logger.getLogger(ClienteDAO.class.getName()).log(Level.SEVERE, null, ex);
    }
